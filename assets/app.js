...
const DB_NAME = 'indicadores_saude_bucal';
const DB_VERSION = 1;
...
const PROCEDURE_RULES = [
  ...
  {re:/ORIENTA(?:C|Ç)AO (?:DE|EM) HIGIENE BUCAL/,code:'01.01.02.010-4',name:'Orientação de higiene bucal',roles:['preventive','b5num','b5den']},
  {re:/ORIENTA(?:C|Ç)AO DE HIGIENIZA(?:C|Ç)AO DE/,code:'01.01.02.012-0',name:'Orientação de higienização de próteses',roles:['preventive','b5num','b5den','b3den']},
  {re:/APLICA(?:C|Ç)AO DE CARIOSTATICO/,code:'01.01.02.005-8',name:'Aplicação de cariostático',roles:['preventive','b5num','b5den','b3den']},
  {re:/APLICA(?:C|Ç)AO DE SELANTE/,code:'01.01.02.006-6',name:'Aplicação de selante',roles:['preventive','b5num','b5den','b3den']},
  {re:/APLICA(?:C|Ç)AO TOPICA DE FLUOR/,code:'01.01.02.007-4',name:'Aplicação tópica de flúor',roles:['preventive','b5num','b5den','b3den']},
  {re:/EVIDENCIA(?:C|Ç)AO DE PLACA/,code:'01.01.02.008-2',name:'Evidenciação de placa bacteriana',roles:['preventive','b5num','b5den','b3den']},
  {re:/PROFILAXIA\s+REMO(?:C|Ç)AO DA PLACA/,code:'03.07.03.004-0',name:'Profilaxia/remoção da placa',roles:['preventive','b5num','b5den','b3den']},
  {re:/RETIRADA DE PONTOS DE CIRURGIAS/,code:'03.01.10.015-6',name:'Retirada de pontos de cirurgias (por paciente)',roles:['preventive']},
  {re:/SELAMENTO PROVISORIO DE CAVIDADE/,code:'01.01.02.009-0',name:'Selamento provisório de cavidade',roles:['b5den','b3den']},
  {re:/CAPEAMENTO PULPAR/,code:'03.07.01.001-5',name:'Capeamento pulpar',roles:['b5den','b3den']},
  {re:/TRATAMENTO INICIAL DO DENTE/,code:'03.07.01.006-6',name:'Tratamento inicial do dente traumatizado',roles:['b5den','b3den']},
  {re:/TRATAMENTO RESTAURADOR/,code:'03.07.01.007-4',name:'Tratamento restaurador atraumático (ART)',roles:['art','restorative','b5den','b3den']},
  {re:/RESTAURA(?:C|Ç)AO DE DENTE PERMANENTE/,code:'',name:'Restauração de dente permanente (subtipo não exibido)',roles:['restorative','b5den','b3den'],ambiguous:true},
  {re:/RESTAURA(?:C|Ç)AO DE DENTE DECIDUO/,code:'',name:'Restauração de dente decíduo (subtipo não exibido)',roles:['restorative','b5den','b3den'],ambiguous:true},
  {re:/ACESSO A POLPA DENTARIA E MEDICACAO/,code:'03.07.02.001-0',name:'Acesso à polpa dentária e medicação',roles:['b5den','b3den']},
  {re:/CURATIVO DE DEMORA/,code:'03.07.02.002-9',name:'Curativo de demora',roles:['b5den','b3den']},
  {re:/PULPOTOMIA/,code:'03.07.02.007-0',name:'Pulpotomia dentária',roles:['b5den','b3den']},
  {re:/RASPAGEM ALISAMENTO SUBGENGIVAIS/,code:'03.07.03.002-4',name:'Raspagem e alisamento subgengivais',roles:['b5den','b3den']},
  {re:/RASPAGEM ALISAMENTO E POLIMENTO/,code:'03.07.03.005-9',name:'Raspagem, alisamento e polimento supragengivais',roles:['b5den','b3den']},
  {re:/TRATAMENTO DE GENGIVITE ULCERATIVA/,code:'03.07.03.006-7',name:'Tratamento de gengivite ulcerativa',roles:['b5den','b3den']},
  {re:/TRATAMENTO DE LESOES DA MUCOSA/,code:'03.07.03.007-5',name:'Tratamento de lesões da mucosa oral',roles:['b5den','b3den']},
  {re:/TRATAMENTO DE PERICORONARITE/,code:'03.07.03.008-3',name:'Tratamento de pericoronarite',roles:['b5den','b3den']},
  {re:/FOTOBIOMODULA(?:C|Ç)AO/,code:'03.07.05.001-7',name:'Fotobiomodulação',roles:['b5den','b3den']},
  {re:/EXODONTIA DE DENTE PERMANENTE/,code:'04.14.02.013-8',name:'Exodontia de dente permanente',roles:['b3num','b3den','b5den']},
  {re:/EXODONTIA MULTIPLA/,code:'04.14.02.014-6',name:'Exodontia múltipla com alveoloplastia',roles:['b3num','b3den']},
  {re:/ADEQUA(?:C|Ç)AO DO COMPORTAMENTO DA/,code:'03.07.01.014-7',name:'Adequação do comportamento da pessoa com deficiência',roles:['b5den']},
  {re:/ADEQUA(?:C|Ç)AO DO COMPORTAMENTO DE CRIAN/,code:'03.07.01.015-5',name:'Adequação do comportamento de crianças',roles:['b5den']}
];
...
function defaultState(){
  const d=new Date();return {
    ...
    gestantes:{manual:[],followups:{},merges:{}},manualOverrides:[],audit:[],lastBackupAt:null,dirty:false,
    preferences:{year:d.getFullYear(),quarter:quarterOfMonth(d.getMonth()+1),month:monthKey(d.getFullYear(),d.getMonth()+1),unit:'',view:'overview',sourceMode:'auto',targetScore:100,pregPopulation:'Presente',pregTeam:'',pregStatus:'',pregFollowup:'',pregOrigin:'',pregPhone:'',pregSearch:''},
    security:{lockEnabled:false},selfTests:null
  };
}
...
async function dbGet(key){const db=await openDB();return new Promise((resolve,reject)=>{const tx=db.transaction(STORE_NAME,'readonly');const r=tx.objectStore(STORE_NAME).get(key);r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error)})}
async function dbPut(key,value){const db=await openDB();return new Promise((resolve,reject)=>{const tx=db.transaction(STORE_NAME,'readwrite');tx.objectStore(STORE_NAME).put(value,key);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error)})}
...
async function persistState(){state.updatedAt=nowISO();const record=state.security?.lockEnabled?await encryptJSON(state,sessionPassword):{encrypted:false,payload:state};await dbPut('state',record)}
const queueSave=()=>{state.dirty=true;clearTimeout(saveTimer);saveTimer=setTimeout(()=>persistState().catch(showError),250)};
...
async function parseProcedurePdf(file,hash,pages){
  ...
  for(const [mk,rows] of Object.entries(monthGroups)){
    ...
    const individual=sum(procs.filter(p=>!/^EVOLUCAO DA ATIVIDADE EM GRUPO/.test(norm(p.descriptionOriginal))).map(p=>p.quantityValid));
    snap.dataByMonth[mk]={kind:'procedure',firstConsultations:firstPeople.size,firstConsultationQuantity:roleQty('first'),treatmentsConcluded:concludedPeople.size,treatmentConcludedQuantity:roleQty('concluded'),preventive:roleQty('preventive'),individualProcedures:individual,art:roleQty('art'),restorative:roleQty('restorative'),b5Numerator:roleQty('b5num'),b5Denominator:roleQty('b5den'),b3Numerator:roleQty('b3num'),b3Denominator:roleQty('b3den'),procedureCounts:procs};
  ...
  snap.validations.push({level:'warning',code:'CBO_NOT_AVAILABLE',message:'O relatório não exibe CBO/INE. Os cálculos federais são prévios e a elegibilidade profissional não foi verificada.'});
...
async function parseGroupPdf(file,hash,pages){
  ...
  for(const ev of events){const d=parseDate(ev.date);if(!d)continue;const mk=monthKey(d.getFullYear(),d.getMonth()+1);snap.dataByMonth[mk]??={kind:'group',supervisedBrushingPresent:0,activities:0,eligibleActivities:0};snap.dataByMonth[mk].activities++;if(/ESCOVACAO SUPERVISIONADA/.test(norm(ev.subject))){snap.dataByMonth[mk].eligibleActivities++;snap.dataByMonth[mk].supervisedBrushingPresent+=Number(ev.present)||0}}
...
async function ensure2IMappings(values){
  const missing=values.filter(v=>state.columnMappings.consulta2i[v]==null);if(!missing.length)return true;
  return new Promise(resolve=>{
    hideLoading();
    const rows=missing.map((v,i)=>`<div class="mapping-row"><div><strong>${esc(v||'(vazio)')}</strong><small>Valor distinto encontrado em “Consulta Saude Bucal”</small></div><select data-map-value="${esc(v)}"><option value="indeterminado">Indeterminado</option><option value="atende" ${norm(v)==='SIM'?'selected':''}>Atende</option><option value="pendente" ${norm(v)==='NAO'?'selected':''}>Pendente</option></select></div>`).join('');
    openModal(`<div class="modal-head"><div><h2 id="modalTitle">Mapear Consulta Saude Bucal</h2><p>Confirme explicitamente como cada valor do Metabase deve ser interpretado.</p></div></div><div class="modal-body"><div class="notice warn"><strong>Não há presunção automática.</strong> As opções estão apenas sugeridas; o mapeamento só será usado após sua confirmação.</div><div style="margin-top:12px">${rows}</div></div><div class="modal-foot"><button class="btn" data-modal-cancel>Cancelar importação</button><button class="btn primary" id="confirm2IMap">Confirmar mapeamento</button></div>`,{wide:false,closable:false});
    document.getElementById('confirm2IMap').onclick=()=>{document.querySelectorAll('[data-map-value]').forEach(s=>state.columnMappings.consulta2i[s.dataset.mapValue]=s.value);audit('2i_mapping_confirmed',{values:missing});closeModal();showLoading('Continuando importação','Aplicando o mapeamento 2I confirmado');resolve(true)};
    document.querySelector('[data-modal-cancel]').onclick=()=>{closeModal();showLoading('Cancelando importação','Nenhum episódio 2I foi gravado');resolve(false)};
  });
}
...
async function parseGestantesCSV(file,hash,text){
  ...
  const data=rows.filter(r=>r.some(v=>String(v).trim()));const distinct=[...new Set(data.map(r=>valueBy(r,map,'Consulta Saude Bucal')))].sort();if(!await ensure2IMappings(distinct))throw new Error('Importação cancelada antes da confirmação do mapeamento 2I.');
...
function aggregateProcedureMonth(mk,unit=state.preferences.unit){
  ...
  const keys=['firstConsultations','firstConsultationQuantity','treatmentsConcluded','treatmentConcludedQuantity','preventive','individualProcedures','art','restorative','b5Numerator','b5Denominator','b3Numerator','b3Denominator'];const out=Object.fromEntries(keys.map(k=>[k,0]));out.snapshots=snaps;out.procedureCounts=[];
...
function municipalComponents(id,mk){
  ...
  if(id==='M2'){numerator=proc?.treatmentsConcluded??null;denominator=proc?.firstConsultations??null;snaps=proc?.snapshots||[];hypothesis='Pessoas distintas pelo nome exibido no PDF no mês.'}
  ...
  if(id==='M4'){numerator=proc?.preventive??null;denominator=proc?.individualProcedures??null;snaps=proc?.snapshots||[];hypothesis='Novo preventivo aumenta simultaneamente numerador e denominador.'}
  if(id==='M5'){numerator=proc?.art??null;denominator=proc?.restorative??null;snaps=proc?.snapshots||[];hypothesis='Perfil B6 de procedimentos restauradores; novo ART aumenta numerador e denominador.'}
...
function federalComponents(id,mk){
  const proc=aggregateProcedureMonth(mk),group=aggregateGroupMonth(mk);let numerator=null,denominator=null,snaps=[],hypothesis='',missing='';
  if(id==='B1'){numerator=proc?.firstConsultations??null;const d=getDenominator('B1','federal',mk);denominator=d?.value??null;snaps=proc?.snapshots||[];hypothesis='Prévia: a janela de 12 meses, INE, CBO e vinculação não foram verificados.';if(!d)missing='Denominador federal B1 não informado.'}
  if(id==='B2'){numerator=proc?.treatmentsConcluded??null;denominator=proc?.firstConsultations??null;snaps=proc?.snapshots||[];hypothesis='Prévia mensal sem validação da janela de 12 meses.'}
  if(id==='B4'){numerator=group?.supervisedBrushingPresent??null;const d=getDenominator('B4','federal',mk);denominator=d?.value??null;snaps=group?.snapshots||[];hypothesis='Presentes agregados; idade, CNS, INE e CBO não verificados.';if(!d)missing='Denominador federal B4 não informado.'}
  if(id==='B5'){numerator=proc?.b5Numerator??null;denominator=proc?.b5Denominator??null;snaps=proc?.snapshots||[];hypothesis='Lista SIGTAP B5; CBO não exibido pelo relatório CELK. O numerador federal permanece separado do M4 municipal.'}
  if(id==='B6'){numerator=proc?.art??null;denominator=proc?.restorative??null;snaps=proc?.snapshots||[];hypothesis='MIAOI e perfil restaurador B6; CBO/INE não exibidos.'}
  if(id==='B3'){numerator=proc?.b3Numerator??null;denominator=proc?.b3Denominator??null;snaps=proc?.snapshots||[];hypothesis='Lista própria da B3; exodontia não integra cálculo municipal.'}
...
function buildDiagnostics(){
  ...
  const mk=state.preferences.month;for(const id of ['M1','M3']){const c=municipalComponents(id,mk);if(!c.denomRecord)list.push({level:'warning',code:`${id}_DEN_MISSING`,message:`${id} sem denominador manual confirmado em ${fmtMonth(mk)}.`})}
  for(const id of ['B1','B4']){const c=federalComponents(id,mk);if(!c.denominator)list.push({level:'info',code:`${id}_DEN_MISSING`,message:`${id} sem denominador federal confirmado em ${fmtMonth(mk)}.`})}
  ...
  const b2=federalComponents('B2',mk);if(b2.result>100)list.push({level:'error',code:'B2_OVER_100',message:'B2 acima de 100%: revisar defasagem entre tratamentos concluídos e primeiras consultas.'});
...
function followupLabel(v){return v==='whatsapp_enviado'?'WhatsApp enviado · aguardando resposta':v==='ok_manual'?'OK · confirmação manual':v==='atende_confirmado'?'Atende · confirmado no Metabase':'Não contatada'}
...
function mergedEpisodes(){
  ...
  for(const e of metabase){const entry=[...manual].find(m=>state.gestantes.merges[m.id]===e.id);if(entry){out.push({...entry,...e,origin:'metabase_manual',manualId:entry.id,metabaseId:e.id,observacao:entry.observacao||'',dataAtividadeManual:entry.dataAtividadeManual||''})}else out.push(e)}
...
function populationFiltered(episodes){const p=state.preferences.pregPopulation;return episodes.filter(e=>!p||e.tipoPopulacao===p||e.origin==='manual')}
function applyPregFilters(episodes){const p=state.preferences;const search=norm(p.pregSearch);return populationFiltered(episodes).filter(e=>(!p.pregTeam||e.equipe===p.pregTeam)&&(!p.pregStatus||e.status2i===p.pregStatus)&&(!p.pregOrigin||e.origin===p.pregOrigin)&&(!p.pregPhone||(p.pregPhone==='valid'?!!e.phoneNormalized:!e.phoneNormalized))&&(!p.pregFollowup||followupFor(e.id).state===p.pregFollowup)&&(!search||norm(`${e.nome} ${e.prontuario}`).includes(search)))}
...
function setFollowup(id,next,note=''){
  const old=followupFor(id),entry={at:nowISO(),from:old.state,to:next,note};state.gestantes.followups[id]={state:next,updatedAt:entry.at,history:[...(old.history||[]),entry]};audit('2i_followup_changed',{episodeId:id,from:old.state,to:next});queueSave();refreshAll();
}
...
function emptyState(title,text,action=true){return `<article class="card empty-state"><div class="large-icon">${icon('upload')}</div><h2>${esc(title)}</h2><p>${esc(text)}</p>${action?'<button class="btn primary" data-action="import">'+icon('upload')+'Importar relatórios</button>':''}</article>`}
function dataQuality(comp){if(comp.result==null)return pill('Dados insuficientes','neutral');if(comp.resultKind==='informado pelo Metabase')return pill('Consolidado informado','info');return pill('Prévia não homologada','warn')}
...
function indicatorCard(comp,scope='municipal'){
  const isM=scope==='municipal',rule=isM?RULESETS.municipal.indicators[comp.id]:RULESETS.federal.indicators[comp.id];const label=isM?(comp.id==='M1'?comp.classification:comp.score!=null?`${fmtNum(comp.score,1)} pts`:'Dados insuficientes'):(comp.classification||'Dados insuficientes');const denomRequired=(isM&&['M1','M3'].includes(comp.id))||(!isM&&['B1','B4'].includes(comp.id));const projection=isM?monthProjection(comp):[];
  return `<article class="card indicator-card" style="--accent:${comp.result==null?'#a2a9bb':label&&statusClass(label)==='bad'?'#e15f41':label&&statusClass(label)==='warn'?'#e7a23b':'#546de5'}"><div class="topline"></div><div class="indicator-head"><div><div class="indicator-id">${comp.id} · ${isM?'MUNICIPAL':'FEDERAL'}</div><div class="indicator-name">${esc(rule.name)}</div></div><button class="info-btn" data-composition="${scope}|${comp.id}|${comp.mk}" aria-label="Como foi calculado?">i</button></div><div class="indicator-result"><strong>${fmtPct(comp.result)}</strong>${pill(label,statusClass(label))}</div><div class="numerator-row"><span>Numerador <strong>${fmtNum(comp.numerator,0)}</strong></span><span>·</span><span>Denominador <strong>${fmtNum(comp.denominator,2)}</strong></span></div>${isM?divergenceNotice(comp):''}${rulerHTML(comp.id,comp.result)}${denomRequired?denomInline(comp.id,scope,comp.mk):''}<div class="indicator-footer"><div class="need">${comp.missing?esc(comp.missing):projection.length?projection.map(p=>`${esc(p.label)}: <strong>${p.value===0?'alcançado':`${p.value} faltante(s)`}</strong>`).join('<br>'):esc(comp.hypothesis)}</div>${dataQuality(comp)}</div></article>`
}
...
function municipalHTML(){
  ...
  return `<div class="notice"><strong>Leitura municipal de Florianópolis.</strong> Quando houver dados do CELK para a competência, eles são obrigatoriamente usados no cálculo. O Metabase permanece como referência consolidada e qualquer divergência é sinalizada.</div><div class="indicator-grid" style="margin-top:16px">${ids.map(id=>indicatorCard(municipalComponents(id,mk))).join('')}</div><article class="card table-card"><div class="table-head"><div><h2 class="section-title">${icon('trend')} Apuração do quadrimestre</h2><div class="section-sub">Meses sem arquivo são distintos de meses com denominador zero.</div></div><label class="field" style="display:flex;align-items:center;grid-template-columns:auto 80px"><span>Nota desejada</span><input id="targetScore" type="number" min="0" max="100" step="1" value="${esc(state.preferences.targetScore)}"></label></div><div class="table-scroll"><table><thead><tr><th>Indicador</th><th>Série mensal</th><th>Parcial</th><th>Projeção</th><th>Auditoria</th></tr></thead><tbody>${qs.map(q=>`<tr><td><strong>${q.id}</strong><br><span class="muted">${esc(RULESETS.municipal.indicators[q.id].name)}</span></td><td style="min-width:310px">${monthBoxes(q)}</td><td>${q.id==='M1'?`${fmtPct(q.result)}<br>${esc(q.classification||'—')}`:`${fmtNum(q.score,1)} pts`}</td><td>${q.id==='M1'?'Faixa aplicada à média percentual':q.needed==null?'—':`${esc(q.status)}<br><span class="muted">${fmtNum(q.needed,1)} pts/mês restantes</span>`}</td><td>${q.id==='M1'?'Prévia analítica':q.diverges?pill('Agregações divergem','warn'):'Leituras coincidem'}</td></tr>`).join('')}</tbody></table></div></article><article class="card table-card"><div class="table-head"><div><h2 class="section-title">${icon('database')} Reconciliação com o Metabase</h2><div class="section-sub">O CELK é a fonte do cálculo quando disponível; o Metabase é mantido como referência de conferência.</div></div></div>${recon.length?`<div class="table-scroll"><table><thead><tr><th>Indicador</th><th>Metabase · referência</th><th>CELK · cálculo</th><th>Diferença</th><th>Numeradores</th><th>Denominadores</th></tr></thead><tbody>${recon.map(r=>`<tr><td>${r.id}</td><td>${fmtPct(r.reported)}</td><td>${fmtPct(r.reconstructed)}</td><td>${fmtPct(r.difference)}</td><td>${fmtNum(r.reportedNumerator)} × ${fmtNum(r.reconstructedNumerator)}</td><td>${fmtNum(r.reportedDenominator,2)} × ${fmtNum(r.reconstructedDenominator,2)}</td></tr>`).join('')}</tbody></table></div>`:'<div class="empty-state"><p>Nenhum consolidado do Metabase disponível para esta competência.</p></div>'}</article>`
}

function federalHTML(){
  const mk=state.preferences.month,ids=['B1','B2','B4','B5','B6'],b3=federalComponents('B3',mk);return `<div class="notice warn"><strong>Série federal mensal.</strong> As Notas B1–B6 não informam como consolidar matematicamente os quatro meses; a ferramenta não inventa resultado federal quadrimestral. Cálculos derivados dos PDFs CELK ficam marcados como prévios quando INE/CBO/CNS não estão disponíveis.</div><div class="indicator-grid" style="margin-top:16px">${ids.map(id=>indicatorCard(federalComponents(id,mk),'federal')).join('')}</div><article class="card panel"><div class="panel-head"><div><h2 class="section-title">${icon('alert')} B3 · Taxa de exodontia</h2><div class="section-sub">Indicador exclusivamente federal e não cobrado no componente municipal analisado.</div></div>${pill(b3.classification||'Dados insuficientes')}</div><div class="split-grid" style="margin:0"><div>${kpi('Taxa federal B3',fmtPct(b3.result),`${fmtNum(b3.numerator)} exodontia(s) ÷ ${fmtNum(b3.denominator)} procedimentos elegíveis`,'#786fa6','tooth')}</div><div class="notice warn"><strong>Faixa normativa atípica:</strong> valores abaixo de 3% também são “Regular”. A ferramenta não recomenda produzir exodontias; solicita auditoria de completude e, em taxas altas, calcula somente a necessidade de ampliar procedimentos elegíveis não exodônticos.</div></div><div class="card-actions"><button class="btn small" data-composition="federal|B3|${mk}">${icon('info')}Ver composição do cálculo</button></div></article><article class="card table-card" style="margin-top:16px"><div class="table-head"><div><h2 class="section-title">${icon('shield')} Faixas federais cadastradas</h2><div class="section-sub">Versão das Notas assinadas em maio de 2026.</div></div></div><div class="table-scroll"><table><thead><tr><th>Indicador</th><th>Ótimo</th><th>Bom</th><th>Suficiente</th><th>Regular</th></tr></thead><tbody><tr><td>B1</td><td>&gt;1,25%</td><td>&gt;0,75% e ≤1,25%</td><td>&gt;0,25% e ≤0,75%</td><td>≤0,25%</td></tr><tr><td>B2</td><td>&gt;75% e ≤100%</td><td>&gt;50% e ≤75%</td><td>&gt;25% e ≤50%</td><td>≤25%</td></tr><tr><td>B3</td><td>≥3% e &lt;10%</td><td>≥10% e &lt;12%</td><td>≥12% e &lt;14%</td><td>&lt;3% ou ≥14%</td></tr><tr><td>B4</td><td>&gt;1%</td><td>&gt;0,5% e ≤1%</td><td>&gt;0,25% e ≤0,5%</td><td>≤0,25%</td></tr><tr><td>B5</td><td>≥65% e ≤85%</td><td>≥55% e &lt;65%</td><td>≥40% e &lt;55%</td><td>&lt;40% ou &gt;85%</td></tr><tr><td>B6</td><td>&gt;8%</td><td>&gt;6% e ≤8%</td><td>&gt;3% e ≤6%</td><td>≤3%</td></tr></tbody></table></div></article>`
}

function pregnancyHTML(){
  const snap=getActive2ISnapshot();
  if(!snap&&!state.gestantes.manual.length)return emptyState('Importe o CSV de gestantes','O arquivo do Metabase forma o panorama automático do 2I. Cadastros manuais ficam em uma camada operacional separada e nunca alteram esse panorama.');
  const metabase=populationFiltered((snap?.episodes||[]).map(e=>({...e,origin:'metabase'})));
  const expanded=populationFiltered(mergedEpisodes());
  const filtered=applyPregFilters(mergedEpisodes());
  const attended=metabase.filter(e=>e.status2i==='atende').length;
  const pending=metabase.filter(e=>e.status2i==='pendente').length;
  const indeterminate=metabase.filter(e=>e.status2i==='indeterminado').length;
  const teams=[...new Set(expanded.map(e=>e.equipe).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'pt-BR'));
  const teamCards=teams.map(team=>{const rows=expanded.filter(e=>e.equipe===team),ok=rows.filter(e=>e.status2i==='atende').length,pct=rows.length?100*ok/rows.length:0;return `<button class="team-card" data-team-filter="${esc(team)}"><div class="team-head"><strong>${esc(team)}</strong><b>${fmtNum(rows.length)}</b></div><div class="progress" style="margin-top:9px"><span style="width:${clamp(pct)}%"></span></div><small>${fmtNum(ok)} atende(m) · ${fmtPct(pct,1)}</small></button>`}).join('');
  const teamOptions=teams.map(t=>`<option ${state.preferences.pregTeam===t?'selected':''}>${esc(t)}</option>`).join('');
  const rows=filtered.map(e=>{const f=followupFor(e.id),manual=e.origin==='manual',status=status2ILabel(e.status2i),weeks=gestationalWeeks(e);return `<tr class="click-row" data-open-episode="${e.id}"><td>${pill(status,statusClass(status))}</td><td>${pill(followupLabel(f.state),f.state==='nao_contatada'?'neutral':f.state==='atende_confirmado'?'success':'info')}</td><td>${pill(manual?'Adicionada manualmente':e.origin==='metabase_manual'?'Manual + Metabase':'Metabase',manual?'warn':'info')}</td><td><strong>${esc(e.nome||'Sem nome')}</strong><br><span class="muted mono">${esc(e.prontuario||String(e.id).slice(0,10))}</span></td><td>${esc(e.equipe||'—')}</td><td>${fmtDate(e.ultimaMenstruacao)}<br><span class="muted">${weeks==null?'IG não calculável':`${weeks} sem.`}</span></td><td>${esc(e.consultaSaudeBucal||'—')}</td><td class="masked-phone">${esc(maskPhone(e.telefone))}</td><td><button class="info-btn" data-open-episode="${e.id}" aria-label="Abrir perfil">${icon('chevron')}</button></td></tr>`}).join('');
  return `<div class="notice"><strong>2I operacional.</strong> O percentual automático usa somente a fotografia do CSV do Metabase e o recorte de população selecionado. Acompanhamentos e inclusões manuais aparecem na lista ampliada, sem alterar o numerador automático.</div><div class="grid-kpis" style="margin-top:16px">${kpi('Panorama do CSV',metabase.length?fmtPct(100*attended/metabase.length,1):'—',`${attended} atende(m) em ${metabase.length} episódio(s)`,'#39b980','heart')}${kpi('Pendentes no CSV',fmtNum(pending),`${indeterminate} indeterminado(s)`,'#e7a23b','clock')}${kpi('Lista operacional',fmtNum(expanded.length),`${state.gestantes.manual.filter(m=>!m.archived&&!state.gestantes.merges[m.id]).length} cadastro(s) manual(is) não mesclado(s)`,'#546de5','users')}${kpi('Com WhatsApp válido',fmtNum(expanded.filter(e=>e.phoneNormalized).length),`${expanded.filter(e=>!e.phoneNormalized).length} sem número válido`,'#3dc1d3','message')}</div><article class="card panel"><div class="panel-head"><div><h2 class="section-title">${icon('users')} Resumo por equipe</h2><div class="section-sub">Clique em uma equipe para filtrar a lista e recalcular o recorte operacional.</div></div><span class="muted">${esc(state.preferences.pregPopulation||'Todos os tipos de população')}</span></div><div class="team-summary">${teamCards||'<span class="muted">Nenhuma equipe disponível.</span>'}</div></article><article class="card table-card" style="margin-top:16px"><div class="table-head"><div><h2 class="section-title">${icon('heart')} Planilha interativa 2I</h2><div class="section-sub">${filtered.length} de ${expanded.length} episódio(s) visível(is). Dados nominais permanecem somente neste navegador.</div></div><div class="card-actions"><button class="btn small" data-add-pregnant>${icon('plus')}Adicionar gestante</button><button class="btn small" data-export-2i>${icon('download')}Exportar</button></div></div><div class="pregnancy-toolbar"><label class="search-box"><span data-icon="search"></span><input id="pregSearch" type="search" value="${esc(state.preferences.pregSearch)}" placeholder="Buscar nome ou prontuário..."></label><select class="filter preg-filter" data-preg-filter="pregPopulation"><option value="">Todas as populações</option>${[...new Set((snap?.episodes||[]).map(e=>e.tipoPopulacao).filter(Boolean))].sort().map(v=>`<option ${state.preferences.pregPopulation===v?'selected':''}>${esc(v)}</option>`).join('')}</select><select class="filter preg-filter" data-preg-filter="pregTeam"><option value="">Todas as equipes</option>${teamOptions}</select><select class="filter preg-filter" data-preg-filter="pregStatus"><option value="">Todos os status</option><option value="atende" ${state.preferences.pregStatus==='atende'?'selected':''}>Atende</option><option value="pendente" ${state.preferences.pregStatus==='pendente'?'selected':''}>Pendente</option><option value="indeterminado" ${state.preferences.pregStatus==='indeterminado'?'selected':''}>Indeterminado</option><option value="sem_metabase" ${state.preferences.pregStatus==='sem_metabase'?'selected':''}>Sem Metabase</option></select><select class="filter preg-filter" data-preg-filter="pregFollowup"><option value="">Todo acompanhamento</option><option value="nao_contatada" ${state.preferences.pregFollowup==='nao_contatada'?'selected':''}>Não contatada</option><option value="whatsapp_enviado" ${state.preferences.pregFollowup==='whatsapp_enviado'?'selected':''}>WhatsApp enviado</option><option value="ok_manual" ${state.preferences.pregFollowup==='ok_manual'?'selected':''}>OK manual</option><option value="atende_confirmado" ${state.preferences.pregFollowup==='atende_confirmado'?'selected':''}>Atende confirmado</option></select><select class="filter preg-filter" data-preg-filter="pregOrigin"><option value="">Todas as origens</option><option value="metabase" ${state.preferences.pregOrigin==='metabase'?'selected':''}>Metabase</option><option value="manual" ${state.preferences.pregOrigin==='manual'?'selected':''}>Manual</option><option value="metabase_manual" ${state.preferences.pregOrigin==='metabase_manual'?'selected':''}>Manual + Metabase</option></select><select class="filter preg-filter" data-preg-filter="pregPhone"><option value="">Todo contato</option><option value="valid" ${state.preferences.pregPhone==='valid'?'selected':''}>WhatsApp válido</option><option value="invalid" ${state.preferences.pregPhone==='invalid'?'selected':''}>Sem WhatsApp válido</option></select>${state.preferences.pregTeam?'<button class="btn small ghost" data-clear-preg-filters>Limpar filtros</button>':''}</div><div class="table-scroll"><table><thead><tr><th>Situação 2I</th><th>Acompanhamento</th><th>Origem</th><th>Gestante / prontuário</th><th>Equipe</th><th>DUM / IG</th><th>Valor original</th><th>Telefone</th><th></th></tr></thead><tbody>${rows||'<tr><td colspan="9" class="muted">Nenhum episódio corresponde aos filtros.</td></tr>'}</tbody></table></div></article>`;
}
...
function openDrawer(html){const b=document.getElementById('drawerBackdrop'),d=document.getElementById('drawer');d.innerHTML=html;b.classList.add('open');b.setAttribute('aria-hidden','false');hydrateIcons(d)}
...
function procedureRolesFor(id){return ({M1:['first'],M2:['first','concluded'],M3:[],M4:['preventive'],M5:['art','restorative'],B1:['first'],B2:['first','concluded'],B3:['b3num','b3den'],B4:[],B5:['b5num','b5den'],B6:['art','restorative']})[id]||[]}
...
function backupState(type='full'){
  ...
  if(type==='analytic'){copy.snapshots=copy.snapshots.filter(s=>s.profile!=='metabase_gestantes_2i');copy.gestantes={manual:[],followups:{},merges:{}};copy.audit=(copy.audit||[]).filter(a=>!String(a.action).startsWith('2i_'));copy.columnMappings={...copy.columnMappings,consulta2i:{}}}
  return copy;
}
...
async function restoreBackup(env,mode){showLoading('Restaurando backup',mode==='replace'?'Substituindo o estado local':'Mesclando dados sem duplicação');try{await dbPut(`pre-restore-${Date.now()}`,{encrypted:false,payload:state});const incoming=migrateState(env.data.state),localSecurity={...state.security};if(mode==='replace'){state=incoming;state.security=localSecurity}else{state.snapshots=mergeArraysBy(state.snapshots,incoming.snapshots,'hash');state.denominators=mergeArraysBy(state.denominators,incoming.denominators,'id');state.gestantes.manual=mergeArraysBy(state.gestantes.manual,incoming.gestantes.manual,'id');state.gestantes.followups={...state.gestantes.followups,...incoming.gestantes.followups};state.gestantes.merges={...state.gestantes.merges,...incoming.gestantes.merges};state.columnMappings.consulta2i={...state.columnMappings.consulta2i,...incoming.columnMappings.consulta2i};state.audit=mergeArraysBy(state.audit,incoming.audit,'id')}audit('backup_restored',{mode,type:env.type});await persistState();pendingBackupFile=null;closeModal();refreshAll();toast('Backup restaurado com sucesso.')}catch(e){showError(e)}finally{hideLoading()}}

function openLockSettings(){openModal(`<div class="modal-head"><div><h2 id="modalTitle">${state.security.lockEnabled?'Alterar bloqueio':'Ativar bloqueio local'}</h2><p>A senha cifra o estado persistido neste navegador.</p></div></div><div class="modal-body"><div class="form-grid"><label class="field full"><span>Nova senha</span><input id="lockPassword" type="password" minlength="8" autocomplete="new-password"></label><label class="field full"><span>Confirmar senha</span><input id="lockPassword2" type="password" minlength="8" autocomplete="new-password"></label></div><div class="notice warn" style="margin-top:12px"><strong>Sem recuperação de senha.</strong> Exporte também um backup cifrado e guarde a senha.</div></div><div class="modal-foot">${state.security.lockEnabled?'<button class="btn danger" data-disable-lock>Desativar bloqueio</button>':''}<button class="btn" data-close-modal>Cancelar</button><button class="btn primary" data-enable-lock>Salvar bloqueio</button></div>`)}
async function enableLock(){const a=document.getElementById('lockPassword').value,b=document.getElementById('lockPassword2').value;if(a.length<8||a!==b){toast('As senhas devem coincidir e ter pelo menos 8 caracteres.');return}sessionPassword=a;state.security.lockEnabled=true;audit('local_lock_enabled');await persistState();closeModal();refreshAll();toast('Bloqueio local ativado.')}
async function disableLock(){state.security.lockEnabled=false;sessionPassword='';audit('local_lock_disabled');await persistState();closeModal();refreshAll();toast('Bloqueio local desativado.')}
async function unlockStoredRecord(record,password){const loaded=await decryptJSON(record,password);sessionPassword=password;state=migrateState(loaded);activeView=state.preferences.view||'overview';refreshAll();closeModal();toast('Ferramenta desbloqueada.')}
function promptStoredUnlock(record){openModal(`<div class="modal-head"><div><h2 id="modalTitle">Indicadores Saúde Bucal bloqueado</h2><p>Informe a senha local para abrir os dados.</p></div></div><div class="modal-body"><label class="field"><span>Senha</span><input id="storedPassword" type="password" autocomplete="current-password"></label><div class="notice warn" style="margin-top:12px"><strong>A senha não pode ser recuperada.</strong> Se necessário, use um backup válido em outro navegador.</div></div><div class="modal-foot"><button class="btn primary" id="storedUnlock">Desbloquear</button></div>`,{closable:false});document.getElementById('storedUnlock').onclick=async()=>{try{await unlockStoredRecord(record,document.getElementById('storedPassword').value)}catch{toast('Senha incorreta ou estado local corrompido.')}}}
...
async function runSelfTests(){const started=performance.now(),results=[];const eq=(a,b,t=1e-9)=>typeof a==='number'&&typeof b==='number'?Math.abs(a-b)<=t:a===b;const add=async(name,fn)=>{try{const v=await fn();if(v!==true)throw new Error(`obtido ${String(v)}`);results.push({name,pass:true})}catch(e){results.push({name,pass:false,error:e.message})}};
  ...
  await add('38. OK manual não altera numerador automático',()=>{const e={status2i:'pendente'},f={state:'ok_manual'};return e.status2i==='pendente'&&f.state==='ok_manual'});
  ...
  await add('50. Backup analítico exclui integralmente 2I',()=>{const b=backupState('analytic');return !b.snapshots.some(s=>s.profile==='metabase_gestantes_2i')&&b.gestantes.manual.length===0&&Object.keys(b.gestantes.followups).length===0});
  await add('51. Manual não altera panorama exclusivo do CSV',()=>{const csv=[{status2i:'atende'}],manual=[{status2i:'sem_metabase'}];return csv.filter(x=>x.status2i==='atende').length===1&&manual.length===1});
...
function switchView(view,{save=true}={}){if(!VIEW_META[view])view='overview';activeView=view;state.preferences.view=view;document.querySelectorAll('.view').forEach(el=>el.classList.toggle('hidden',el.id!==`view-${view}`));document.querySelectorAll('[data-view]').forEach(el=>el.classList.toggle('active',el.dataset.view===view));const [title,sub]=VIEW_META[view];document.getElementById('pageTitle').textContent=title;document.getElementById('pageSubtitle').textContent=sub;document.getElementById('eyebrow').textContent=`Indicadores / ${view==='overview'?'Saúde Bucal':title.split('·')[0].trim()}`;if(save)queueSave();document.querySelector('.workspace').scrollTop=0}
function refreshAll(){
  fillContextFilters();
  document.getElementById('view-overview').innerHTML=overviewHTML();document.getElementById('view-municipal').innerHTML=municipalHTML();document.getElementById('view-federal').innerHTML=federalHTML();document.getElementById('view-pregnant').innerHTML=pregnancyHTML();document.getElementById('view-imports').innerHTML=importsHTML();document.getElementById('view-diagnostics').innerHTML=diagnosticsHTML();document.getElementById('view-settings').innerHTML=settingsHTML();
  const newest=[...state.snapshots].sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt))[0];document.getElementById('lastUpdate').textContent=newest?`Atualizado ${fmtDateTime(newest.createdAt)}`:'Nenhum relatório importado';document.getElementById('snapshotCount').textContent=`${state.snapshots.length} snapshot${state.snapshots.length===1?'':'s'}`;document.getElementById('backupStatus').textContent=state.lastBackupAt?`Backup ${fmtDateTime(state.lastBackupAt)}`:'Backup ainda não exportado';const d=buildDiagnostics(),badge=document.getElementById('diagnosticBadge'),n=d.filter(x=>x.level==='error'||x.level==='warning').length;badge.textContent=n;badge.classList.toggle('hidden',!n);hydrateIcons();switchView(activeView,{save:false});
}
...
function setupEvents(){
  document.addEventListener('click',async ev=>{const el=ev.target.closest('button,[data-go],[data-open-episode],[data-open-snapshot],[data-team-filter]');if(!el)return;
    if(el.matches('[data-close-modal]'))return closeModal();if(el.matches('[data-close-drawer]'))return closeDrawer();if(el.dataset.view)return switchView(el.dataset.view);if(el.dataset.go)return switchView(el.dataset.go);if(el.dataset.action==='import')return document.getElementById('fileInput').click();
    ...
    if(el.dataset.saveDenom){const [id,scope]=el.dataset.saveDenom.split('|'),input=document.querySelector(`[data-denom-input="${id}|${scope}"]`);return openDenominatorModal(id,scope,input?.value||'')}
    ...
    if(el.hasAttribute('data-add-pregnant'))return openManualPregnant();if(el.dataset.openEpisode)return openEpisode(el.dataset.openEpisode);if(el.dataset.teamFilter){state.preferences.pregTeam=el.dataset.teamFilter;refreshAll();return}if(el.hasAttribute('data-clear-preg-filters')){for(const k of ['pregTeam','pregStatus','pregFollowup','pregOrigin','pregPhone','pregSearch'])state.preferences[k]='';queueSave();return refreshAll()}
    ...
    if(el.dataset.followup){const [id,next]=el.dataset.followup.split('|');return setFollowup(id,next)}if(el.dataset.mergeManual){const [m,e]=el.dataset.mergeManual.split('|');return mergeManual(m,e)}if(el.dataset.editManual){closeDrawer();return openManualPregnant(el.dataset.editManual)}if(el.dataset.archiveManual){const m=state.gestantes.manual.find(x=>x.id===el.dataset.archiveManual);if(m){m.archived=true;audit('2i_manual_archived',{manualId:m.id});closeDrawer();refreshAll();toast('Cadastro manual arquivado.')}return}
    if(el.hasAttribute('data-export-procedures'))return exportProcedures();if(el.hasAttribute('data-export-2i'))return export2IModal();if(el.dataset.export2iMode)return export2I(el.dataset.export2iMode);if(el.hasAttribute('data-run-tests')){showLoading('Executando 54 testes','Fórmulas, faixas, privacidade e 2I');try{const r=await runSelfTests();toast(`${r.passed}/${r.total} testes passaram.`)}finally{hideLoading()}return}
    if(el.hasAttribute('data-create-backup'))return createBackupFromModal();if(el.hasAttribute('data-restore-backup'))return document.getElementById('backupInput').click();if(el.hasAttribute('data-unlock-backup'))return processPendingBackup(document.getElementById('restorePassword')?.value||'');if(el.hasAttribute('data-toggle-lock'))return openLockSettings();if(el.hasAttribute('data-enable-lock'))return enableLock();if(el.hasAttribute('data-disable-lock'))return disableLock();
  });
  document.getElementById('importBtn').onclick=()=>document.getElementById('fileInput').click();document.getElementById('fileInput').onchange=e=>importFiles(e.target.files);document.getElementById('backupBtn').onclick=openBackupModal;document.getElementById('printBtn').onclick=()=>window.print();document.getElementById('lockBtn').onclick=async()=>{if(state.security.lockEnabled){await persistState();location.reload()}else openLockSettings()};
  ...
  document.addEventListener('change',e=>{if(e.target.matches('.preg-filter')){state.preferences[e.target.dataset.pregFilter]=e.target.value;queueSave();refreshAll()}if(e.target.id==='sourceMode')updatePreference('sourceMode',e.target.value);if(e.target.id==='settingsTarget'||e.target.id==='targetScore')updatePreference('targetScore',clamp(Number(e.target.value),0,100))});
  document.addEventListener('input',debounce(e=>{if(e.target.id==='pregSearch'){state.preferences.pregSearch=e.target.value;queueSave();document.getElementById('view-pregnant').innerHTML=pregnancyHTML();hydrateIcons(document.getElementById('view-pregnant'))}},250));
