...
async function parseGestantesCSV(file,hash,text){
  ...
  for(let line=0;line<data.length;line++){
    ...
    record.status2i=state.columnMappings.consulta2i[record.consultaSaudeBucal]||'indeterminado';record.phoneNormalized=normalizePhone(record.telefone);record.id=await episodeIdFor(record);
...
function indicatorCard(comp,scope='municipal'){
  const isM=scope==='municipal',rule=isM?RULESETS.municipal.indicators[comp.id]:RULESETS.federal.indicators[comp.id];const label=isM?(comp.id==='M1'?comp.classification:comp.score!=null?`${fmtNum(comp.score,1)} pts`:'Dados insuficientes'):(comp.classification||'Dados insuficientes');const denomRequired=isM&&['M1','M3'].includes(comp.id);const denomShared=!isM&&['B1','B4'].includes(comp.id);const projection=isM?monthProjection(comp):[];
  return `<article class="card indicator-card" style="--accent:${comp.result==null?'#a2a9bb':label&&statusClass(label)==='bad'?'#e15f41':label&&statusClass(label)==='warn'?'#e7a23b':'#546de5'}"><div class="topline"></div><div class="indicator-head"><div><div class="indicator-id">${comp.id} · ${isM?'MUNICIPAL':'FEDERAL'}</div><div class="indicator-name">${esc(rule.name)}</div></div><button class="info-btn" data-composition="${scope}|${comp.id}|${comp.mk}" aria-label="Como foi calculado?">i</button></div><div class="indicator-result"><strong>${fmtPct(comp.result)}</strong>${pill(label,statusClass(label))}</div><div class="numerator-row"><span>Numerador <strong>${fmtNum(comp.numerator,0)}</strong></span><span>·</span><span>Denominador <strong>${fmtNum(comp.denominator,2)}</strong></span></div>${isM?divergenceNotice(comp):''}${rulerHTML(comp.id,comp.result)}${denomRequired?denomInline(comp.id,scope,comp.mk):denomShared?sharedDenomNote(comp.mirrorOf):''}<div class="indicator-footer"><div class="need">${comp.missing?esc(comp.missing):projection.length?projection.map(p=>`${esc(p.label)}: <strong>${p.value===0?'alcançado':`${p.value} faltante(s)`}</strong>`).join('<br>'):esc(comp.hypothesis)}</div>${dataQuality(comp)}</div></article>`
}
...
function pregnancyHTML(){
  ...
  const metabase=(snap?.episodes||[]).map(e=>({...e,origin:'metabase'}));
...
function importsHTML(){
  ...
  const procRows=(proc?.procedureCounts||[]).sort((a,b)=>b.quantityValid-a.quantityValid).map(p=>`<tr><td><strong>${esc(p.descriptionNormalized)}</strong><br><span class="muted">${esc(p.descriptionOriginal)}</span></td><td class="mono">${esc(p.sigtap||'—')}</td><td class="num">${fmtNum(p.quantityRaw,2)}</td><td class="num">${fmtNum(Math.max(0,p.quantityRaw-p.quantityValid),2)}</td><td class="num"><strong>${fmtNum(p.quantityValid,2)}</strong></td><td>${(p.roles||[]).length?(p.roles||[]).map(r=>pill(r,'neutral')).join(' '):p.outOfScope?pill('Fora do escopo','info'):p.unrecognized?pill('Não identificado','warn'):'—'}</td><td>${(p.pages||[]).map(x=>`p.${x}`).join(', ')}</td></tr>`).join('');
...
function backupState(type='full'){
  ...
  if(type==='analytic'){copy.snapshots=copy.snapshots.filter(s=>s.profile!=='metabase_gestantes_2i');copy.gestantes={manual:[],followups:{},merges:{},excluded:{}};copy.audit=(copy.audit||[]).filter(a=>!String(a.action).startsWith('2i_'));copy.columnMappings={...copy.columnMappings,consulta2i:{}}}
  return copy;
}
...
async function runSelfTests(){const started=performance.now(),results=[];const eq=(a,b,t=1e-9)=>typeof a==='number'&&typeof b==='number'?Math.abs(a-b)<=t:a===b;const add=async(name,fn)=>{try{const v=await fn();if(v!==true)throw new Error(`obtido ${String(v)}`);results.push({name,pass:true})}catch(e){results.push({name,pass:false,error:e.message})}};
  ...
  await add('89. KPI de gestantes na Visão Geral mostra a contagem no texto (atendidas do total com pré-natal aberto), sem mudar a fórmula do percentual',()=>{const src=overviewHTML.toString();return src.includes('gestante(s) atendida(s) do total de')&&src.includes('com pré-natal aberto')&&src.includes('100*attended/episodes.length')});
...
function setupEvents(){
  document.addEventListener('click',async ev=>{const el=ev.target.closest('button,[data-go],[data-open-episode],[data-open-snapshot],[data-team-filter],[data-overview-scope]');if(!el)return;
    ...
    if(el.hasAttribute('data-export-procedures'))return exportProcedures();if(el.hasAttribute('data-export-2i'))return export2IModal();if(el.dataset.export2iMode)return export2I(el.dataset.export2iMode);if(el.hasAttribute('data-run-tests')){showLoading('Executando 90 testes','Fórmulas, faixas, privacidade e 2I');try{const r=await runSelfTests();toast(`${r.passed}/${r.total} testes passaram.`)}finally{hideLoading()}return}
