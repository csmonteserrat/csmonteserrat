...
function followupLabel(v){return v==='whatsapp_enviado'?'WhatsApp enviado · aguardando resposta':v==='busca_ativa_solicitada'?'Busca ativa solicitada':v==='ok_manual'?'OK · confirmação manual':v==='atende_confirmado'?'Atende · confirmado no Metabase':'Não contatada'}
function followupShortLabel(v){return v==='whatsapp_enviado'?'Tentativa de contato · WhatsApp':v==='busca_ativa_solicitada'?'Busca ativa solicitada':''}
const FOLLOWUP_COLORS={nao_contatada:{text:'#697386',bg:'#f0f2f6'},whatsapp_enviado:{text:'#3a52c4',bg:'#eef0ff'},busca_ativa_solicitada:{text:'#b0356b',bg:'#fdecf3'},ok_manual:{text:'#247a4b',bg:'#edf9f3'},atende_confirmado:{text:'#1f7a80',bg:'#e7f7f8'}};
...
function openEpisode(id){const e=mergedEpisodes().find(x=>x.id===id||x.manualId===id);if(!e)return;const f=followupFor(e.id),match=e.origin==='manual'?findManualMatch(e,getActive2ISnapshot()?.episodes||[]):null,weeks=gestationalWeeks(e),address=[e.tipoLogradouro,e.logradouro,e.numero,e.complemento,e.bairro].filter(Boolean).join(', '),excl=state.gestantes.excluded[e.id];openDrawer(`<div class="drawer-head"><div><h2>${esc(e.nome||'Gestante')}</h2><p>${esc(status2ILabel(e.status2i))} · ${esc(e.origin==='manual'?'Adicionada manualmente':e.origin==='metabase_manual'?'Manual + Metabase':'Metabase')}${esc(pregnancyStage(e)==='finalizada'?' · Gestação finalizada':' · Gestação ativa')}</p></div><button class="close-btn" data-close-drawer>${icon('close')}</button></div>${excl?`<div class="notice danger" style="margin-top:14px"><strong>Excluída da lista operacional</strong> em ${fmtDateTime(excl.at)}${excl.reason?` · ${esc(excl.reason)}`:''}</div>`:''}<div class="drawer-summary"><span class="chip-item"><span class="chip-label">Prontuário</span><strong class="mono">${esc(e.prontuario||'ID local temporário')}</strong></span><span class="chip-item"><span class="chip-label">Idade</span><strong>${ageAt(e)==null?'—':`${ageAt(e)} anos`}</strong></span><span class="chip-item"><span class="chip-label">Equipe</span><strong>${esc(e.equipe||'—')}</strong></span><span class="chip-item"><span class="chip-label">DUM</span><strong>${fmtDate(e.ultimaMenstruacao)}</strong></span><span class="chip-item"><span class="chip-label">IG</span><strong>${weeks==null?'—':`${weeks} sem.`}</strong></span><button class="btn small ghost" data-copy-name="${esc(e.id)}">${icon('copy')}Copiar nome</button></div><div class="drawer-contact-line"><div class="cl-num">${esc(e.telefone||'Telefone não informado')}</div><div class="drawer-actions" style="margin-top:0"><button class="btn small success" data-open-whatsapp="${esc(e.id)}" ${e.phoneNormalized?'':'disabled'}>${icon('message')}Abrir WhatsApp</button><button class="btn small info" data-followup="${esc(e.id)}|whatsapp_enviado" ${e.phoneNormalized?'':'disabled'}>Registrar envio</button></div></div><div class="drawer-section" style="margin-top:18px"><h3>Acompanhamento</h3><div class="notice" style="border-left-color:${followupColor(f.state).text};background:${followupColor(f.state).bg}"><strong style="color:${followupColor(f.state).text}">${esc(followupLabel(f.state))}</strong>${f.updatedAt?` · ${fmtDateTime(f.updatedAt)}`:''}</div><div class="drawer-actions"><button class="btn small" data-followup="${esc(e.id)}|ok_manual">OK · atendida manualmente</button><button class="btn small" data-followup="${esc(e.id)}|atende_confirmado">Atende · confirmado</button><button class="btn small busca-ativa" data-followup="${esc(e.id)}|busca_ativa_solicitada">Busca ativa solicitada</button><button class="btn small ghost" data-followup="${esc(e.id)}|nao_contatada">Reiniciar</button></div></div>${match?`<div class="drawer-section"><h3>Reconciliação necessária</h3><div class="notice warn"><strong>Possível correspondência por prontuário + episódio.</strong> Nome isolado nunca é usado para mesclar.</div><div class="facts" style="margin-top:8px"><div class="fact"><span>Manual</span><strong>${esc(e.prontuario||'ID local')}</strong></div><div class="fact"><span>Metabase</span><strong>${esc(match.prontuario)} · ${fmtDate(episodeAnchor(match))}</strong></div></div><button class="btn primary small" style="margin-top:9px" data-merge-manual="${esc(e.id)}|${esc(match.id)}">Confirmar mesclagem</button></div>`:''}<div class="disclosure"><button class="disclosure-btn" data-toggle-details><span>Mais detalhes — endereço, gestação, saúde bucal, histórico${e.origin==='manual'?', cadastro manual':''}, lista operacional</span><span class="chev">${icon('chevron')}</span></button><div class="disclosure-body"><div class="drawer-mini-section"><h4>Endereço e gestação</h4><div class="facts"><div class="fact"><span>Endereço</span><strong>${esc(address||'Não informado')}</strong></div><div class="fact"><span>Telefone normalizado</span><strong>${esc(e.phoneNormalized||'Inválido ou sem DDD')}</strong></div><div class="fact"><span>DPP</span><strong>${fmtDate(e.dataProvParto)}</strong></div><div class="fact"><span>Parto</span><strong>${fmtDate(e.dataParto)}</strong></div><div class="fact"><span>Nascimento</span><strong>${fmtDate(e.dataNascimento)}</strong></div></div></div><div class="drawer-mini-section"><h4>Saúde bucal e origem</h4><div class="facts"><div class="fact"><span>Valor original do CSV</span><strong>${esc(e.consultaSaudeBucal||'Não veio do Metabase')}</strong></div><div class="fact"><span>Interpretação 2I</span><strong>${esc(status2ILabel(e.status2i))}</strong></div><div class="fact"><span>Atividade manual</span><strong>${fmtDate(e.dataAtividadeManual)}</strong></div><div class="fact"><span>Observação manual</span><strong>${esc(e.observacao||'—')}</strong></div>${e.notaLocal?`<div class="fact"><span>Nota local</span><strong>${esc(e.notaLocal)}</strong></div>`:''}</div></div><div class="drawer-mini-section"><h4>Histórico de acompanhamento</h4><div class="timeline">${(f.history||[]).slice().reverse().map(h=>`<div class="timeline-item"><i class="timeline-dot" style="background:${followupColor(h.to).text}"></i><div class="timeline-copy"><strong style="color:${followupColor(h.to).text}">${esc(followupLabel(h.to))}</strong><span>${fmtDateTime(h.at)}${h.note?` · ${esc(h.note)}`:''}</span></div></div>`).join('')||'<span class="muted">Sem histórico.</span>'}</div></div>${e.origin!=='manual'?`<div class="drawer-mini-section"><h4>Corrigir dados</h4><div class="notice">Ajuste local de telefone, DUM/DPP/parto e nota — não altera a fotografia original do CSV do Metabase, fica registrado como correção manual.</div><div class="drawer-actions" style="margin-top:9px"><button class="btn small" data-edit-contact="${esc(e.id)}">Editar telefone/gestação</button></div></div>`:''}${e.origin==='manual'?`<div class="drawer-mini-section"><h4>Gerenciar cadastro manual</h4><div class="drawer-actions"><button class="btn small" data-edit-manual="${esc(e.id)}">Editar</button><button class="btn small danger" data-archive-manual="${esc(e.id)}">Arquivar</button></div></div>`:''}<div class="drawer-mini-section"><h4>Lista operacional</h4><div class="drawer-actions">${excl?`<button class="btn small" data-restore-episode="${esc(e.id)}">Restaurar na lista</button>`:`<button class="btn small danger" data-exclude-episode="${esc(e.id)}">Excluir da lista</button>`}</div><small class="muted">Não apaga o cadastro nem altera o panorama do CSV — só entra ou sai da lista operacional.</small></div></div></div>`)}
...
async function runSelfTests(){const started=performance.now(),results=[];const eq=(a,b,t=1e-9)=>typeof a==='number'&&typeof b==='number'?Math.abs(a-b)<=t:a===b;const add=async(name,fn)=>{try{const v=await fn();if(v!==true)throw new Error(`obtido ${String(v)}`);results.push({name,pass:true})}catch(e){results.push({name,pass:false,error:e.message})}};
  ...
  await add('116. Lista de gestantes mostra um selo de tentativa de contato por WhatsApp e de busca ativa solicitada, numa cor própria, quando o acompanhamento estiver nesses estados',()=>{const src=pregnancyHTML.toString();return !!(src.includes('preg-tag')&&src.includes("followupColor(f.state)")&&src.includes("['whatsapp_enviado','busca_ativa_solicitada']")&&followupShortLabel('whatsapp_enviado')&&followupShortLabel('busca_ativa_solicitada'))});
  await add('117. Gaveta da gestante simplificada: resumo compacto no topo, Acompanhamento logo em seguida, e endereço/saúde bucal/histórico só aparecem dentro de "Mais detalhes", recolhido por padrão',()=>{const src=openEpisode.toString();const iSummary=src.indexOf('drawer-summary'),iAcomp=src.indexOf('<h3>Acompanhamento'),iToggle=src.indexOf('data-toggle-details'),iAddr=src.indexOf('Endereço e gestação'),iSaude=src.indexOf('Saúde bucal e origem'),iHist=src.indexOf('Histórico de acompanhamento');return iSummary>-1&&iAcomp>-1&&iToggle>-1&&iAddr>-1&&iSaude>-1&&iHist>-1&&iSummary<iAcomp&&iAcomp<iToggle&&iToggle<iAddr&&iToggle<iSaude&&iToggle<iHist&&!src.includes('class="disclosure-body open"')});
...
function setupEvents(){
  document.addEventListener('click',async ev=>{const el=ev.target.closest('button,[data-go],[data-open-episode],[data-open-snapshot],[data-team-filter],[data-overview-scope]');if(!el)return;
    ...
    if(el.dataset.followup){const [id,next]=el.dataset.followup.split('|');return setFollowup(id,next)}if(el.dataset.mergeManual){const [m,e]=el.dataset.mergeManual.split('|');return mergeManual(m,e)}if(el.dataset.editManual){closeDrawer();return openManualPregnant(el.dataset.editManual)}if(el.dataset.editContact){closeDrawer();return openEditContact(el.dataset.editContact)}if(el.dataset.archiveManual){const m=state.gestantes.manual.find(x=>x.id===el.dataset.archiveManual);if(m){m.archived=true;audit('2i_manual_archived',{manualId:m.id});closeDrawer();refreshAll();toast('Cadastro manual arquivado.')}return}if(el.dataset.excludeEpisode){closeDrawer();return openExcludeEpisode(el.dataset.excludeEpisode)}if(el.dataset.restoreEpisode)return restoreEpisode(el.dataset.restoreEpisode);
    if(el.hasAttribute('data-export-procedures'))return exportProcedures();if(el.hasAttribute('data-export-2i'))return export2IModal();if(el.dataset.export2iMode)return export2I(el.dataset.export2iMode);if(el.hasAttribute('data-run-tests')){showLoading('Executando 118 testes','Fórmulas, faixas, privacidade e 2I');try{const r=await runSelfTests();toast(`${r.passed}/${r.total} testes passaram.`)}finally{hideLoading()}return}
