...
function cumulativeFederal(id,months){
  ...
  if(['B1','B4'].includes(id)){
    // B1/B4 espelham M1/M3 (denominador de população/faixa etária, não um fluxo mensal) — mesma correção da v1.38
    // em cumulativeMunicipal, pedido explícito do usuário: denominador fixo no mês em foco, só o numerador soma.
    const ref=state.preferences.month,refComp=federalComponents(id,ref),denominator=refComp.denominator;
...
function denomInline(id,scope,mk){
  const record=getDenominator(id,scope,mk),suggestion=scope==='municipal'&&['M1','M3'].includes(id)?suggestedDenominator(id,mk):null;
  const isM1=scope==='municipal'&&id==='M1',popInput=isM1?activePopulationInput(mk):null,popSuggestion=isM1?suggestedM1FromPopulation(mk):null;
  return `<div class="denom-inline"><label>${scope==='municipal'?(id==='M1'?'Denominador do indicador 1':'Denominador do indicador 3'):`Denominador ${id}`} · ${fmtMonth(mk)}</label><div class="denom-controls"><input inputmode="decimal" data-denom-input="${id}|${scope}" value="${record?esc(record.value):''}" placeholder="Informar valor"><button class="btn small" data-save-denom="${id}|${scope}">Salvar</button></div>${record?`<small class="muted">${esc(record.origin)} · vigência ${fmtMonth(record.start)}–${fmtMonth(record.end)}</small>`:''}${suggestion!=null&&(!record||Number(record.value)!==Number(suggestion))?`<button class="link-btn" data-use-suggestion="${id}|${scope}|${suggestion}">Sugestão do Metabase: ${fmtNum(suggestion,2)} · usar</button>`:''}${popSuggestion!=null&&(!record||Number(record.value)!==Number(popSuggestion))?`<button class="link-btn" data-use-suggestion="${id}|${scope}|${popSuggestion}">Sugestão pela população ativa: ${fmtNum(popSuggestion,2)} (${fmtNum(popInput.totalPopulation,0)} ÷ ${popInput.esfCount} ESF × ${popInput.dentistCount} dentistas) · usar</button>`:''}${isM1?`<small class="muted" style="display:block;margin-top:4px">População ativa: baixe o CSV no <a href="${POPULATION_CSV_SOURCE_URL}" target="_blank" rel="noopener">Data Studio</a> e importe junto com os demais relatórios para sugerir este denominador automaticamente.</small>`:''}</div>`
}
...
function indicatorCard(comp,scope='municipal'){
  const isM=scope==='municipal',isB3=comp.id==='B3',rule=isM?RULESETS.municipal.indicators[comp.id]:RULESETS.federal.indicators[comp.id];const label=isM?(comp.id==='M1'?comp.classification:comp.result!=null?zoneLabel(comp.id,comp.result):'Dados insuficientes'):(comp.classification||'Dados insuficientes');const muniPillClass=isM?(comp.id==='M1'?statusClass(label):zonePillClass(comp.id,comp.result)):null;const denomRequired=isM&&['M1','M3'].includes(comp.id);const denomShared=!isM&&['B1','B4'].includes(comp.id);const projection=isM?monthProjection(comp):[];
  const accent=isB3?FEDERAL_B3_ACCENT:!isM?(comp.result==null?'#a2a9bb':federalZoneColor(comp.id,comp.result)):(comp.result==null?'#a2a9bb':muniPillClass==='bad'?'#f0483e':muniPillClass==='warn'?'#f7821f':muniPillClass==='good'?'#2f80ed':'#2cc08b');
  const resultPill=isM?pill(label,muniPillClass):federalPill(label);
...
function federalQuadrimestralOutlook(id,q){
  ...
  if(cum.result==null){
    ...
    if(fixedDen)return {cls:'neutral',label:'Sem dados suficientes',detail:`Sem denominador confirmado de ${id==='B1'?'M1':'M3'} para ${fmtMonth(ref,true)} (mês em foco) — o acumulado de ${id} usa esse valor fixo, não a soma dos 4 meses.`};
  ...
  const detail=fixedDen
    ?`Acumulado do quadrimestre: ${fmtPct(cum.result,pctDecimals(id))} (soma dos numeradores dos meses com dado = ${fmtNum(cum.numerator,0)} ÷ denominador fixo de ${fmtMonth(ref,true)} = ${fmtNum(cum.denominator,2)}), classificado como ${label} pela Nota ${id}. O denominador não é somado entre os meses — é uma medida de população/faixa etária (a mesma usada por ${id==='B1'?'M1':'M3'} no painel Municipal), não um fluxo mensal.`
...
async function runSelfTests(){const started=performance.now(),results=[];const eq=(a,b,t=1e-9)=>typeof a==='number'&&typeof b==='number'?Math.abs(a-b)<=t:a===b;const add=async(name,fn)=>{try{const v=await fn();if(v!==true)throw new Error(`obtido ${String(v)}`);results.push({name,pass:true})}catch(e){results.push({name,pass:false,error:e.message})}};
  ...
  await add('169. federalQuadrimestralOutlook devolve rótulo/classe de pill válidos para os 6 indicadores federais, com ou sem dados carregados, e o detalhe deixa explícito o motivo (cálculo de conveniência para a maioria; denominador fixo do mês em foco para B1/B4)',()=>{const classes=['success','bad','neutral','warn','good'];return ['B1','B2','B3','B4','B5','B6'].every(id=>{const q=quarterFederal(id),r=federalQuadrimestralOutlook(id,q);return classes.includes(r.cls)&&typeof r.label==='string'&&r.label.length>0&&typeof r.detail==='string'&&(r.detail.includes('conveniência')||r.detail.includes('dados suficientes')||r.detail.includes('denominador'))})});
  ...
  await add('175. denomInline mostra a sugestão pela população ativa e o link do Data Studio só para M1 — nunca para M3 (pedido explícito do usuário de não usar esse CSV em M3/B4) — e a renderização em si não troca sozinha um valor diferente já confirmado manualmente para a mesma vigência (o campo continua mostrando esse valor; ver teste 179 para a confirmação automática ao preencher o modal de ESF/dentistas)',()=>{const beforePop=state.populationInputs.length,beforeDenom=state.denominators.length,u=state.preferences.unit,mk=state.preferences.month;try{state.populationInputs.push({id:'tpop2_selftest',snapshotId:'x',fileName:'pop.csv',unit:u,totalPopulation:10000,esfCount:5,dentistCount:2,start:mk,end:mk,createdAt:nowISO()});state.denominators.push({id:'tden2_selftest',indicator:'M1',scope:'municipal',value:999,start:mk,end:mk,unit:u,origin:'teste',confirmed:true,updatedAt:nowISO(),ruleVersion:RULE_VERSION});const htmlM1=denomInline('M1','municipal',mk),htmlM3=denomInline('M3','municipal',mk);return htmlM1.includes('Sugestão pela população ativa')&&htmlM1.includes(POPULATION_CSV_SOURCE_URL)&&htmlM1.includes('value="999"')&&!htmlM3.includes('Sugestão pela população ativa')&&!htmlM3.includes(POPULATION_CSV_SOURCE_URL)}finally{state.populationInputs.length=beforePop;state.denominators.length=beforeDenom}});
