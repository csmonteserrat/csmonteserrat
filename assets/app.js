...
function openPopulationInputsModal(snap){
  ...
  document.getElementById('popForm').onsubmit=e=>{
    ...
    if(existing){existing.esfCount=esfCount;existing.dentistCount=dentistCount;existing.start=start;existing.end=end;existing.totalPopulation=snap.population.totalPopulation;existing.updatedAt=nowISO()}
    else state.populationInputs.push({id:uuid(),snapshotId:snap.id,fileName:snap.fileName,unit:snap.unit||state.preferences.unit,totalPopulation:snap.population.totalPopulation,esfCount,dentistCount,start,end,createdAt:nowISO()});
    audit('population_input_confirmed',{snapshotId:snap.id,esfCount,dentistCount,start,end});
    closeModal();queueSave();refreshAll();toast('População ativa confirmada. A sugestão de denominador de M1/B1 já aparece no painel Municipal.');
  };
}
...
function setupEvents(){
  document.addEventListener('click',async ev=>{const el=ev.target.closest('button,[data-go],[data-open-episode],[data-open-snapshot],[data-team-filter],[data-overview-scope]');if(!el)return;
    ...
    if(el.hasAttribute('data-export-procedures'))return exportProcedures();if(el.hasAttribute('data-export-2i'))return export2IModal();if(el.dataset.export2iMode)return export2I(el.dataset.export2iMode);if(el.hasAttribute('data-run-tests')){showLoading('Executando 178 testes','Fórmulas, faixas, privacidade e 2I');try{const r=await runSelfTests();toast(`${r.passed}/${r.total} testes passaram.`)}finally{hideLoading()}return}
