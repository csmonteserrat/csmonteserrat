...
function pregnancyHTML(){
  ...
  const rows=sorted.map(e=>{const attended=isAttended(e),weeks=gestationalWeeks(e),priority=isPriority2I(e),badgeColor=attended?'#39b980':priority?'#e15f41':'#e7a23b',badgeLabel=attended?'Atendida':priority?'Pendente · 3º trimestre':'Pendente';return `<div class="rowcard click-row" data-open-episode="${e.id}"><div class="rc-left"><strong>${esc(e.nome||'Sem nome')}</strong><span class="muted mono">${esc(e.prontuario||String(e.id).slice(0,10))} · Equipe ${esc(e.equipe||'—')}</span></div><div class="rc-mid">DUM ${fmtDate(e.ultimaMenstruacao)} · ${weeks==null?'IG não calculável':`${weeks} sem.`}</div><div class="rc-right"><span class="badge" style="background:${badgeColor}">${esc(badgeLabel)}</span><button class="info-btn" data-open-episode="${e.id}" aria-label="Abrir perfil">${icon('chevron')}</button></div></div>`}).join('');
...
function setupEvents(){
  document.addEventListener('click',async ev=>{const el=ev.target.closest('button,[data-go],[data-open-episode],[data-open-snapshot],[data-team-filter],[data-overview-scope]');if(!el)return;
    ...
    if(el.hasAttribute('data-export-procedures'))return exportProcedures();if(el.hasAttribute('data-export-2i'))return export2IModal();if(el.dataset.export2iMode)return export2I(el.dataset.export2iMode);if(el.hasAttribute('data-run-tests')){showLoading('Executando 110 testes','Fórmulas, faixas, privacidade e 2I');try{const r=await runSelfTests();toast(`${r.passed}/${r.total} testes passaram.`)}finally{hideLoading()}return}
